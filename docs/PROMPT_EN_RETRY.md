# Prompt for BDD → IBDD Translation (RETRY MODE)

## ⚠️ RETRY MODE ACTIVE

You are attempting to correct a previous translation that failed to parse correctly.

### Previous Attempt Summary: todo: translate
{error_analysis}

### Your Task:
Carefully review the error analysis above and generate a **corrected IBDD translation** that:
1. Fixes the specific parsing errors identified
2. Follows all grammar rules strictly
3. Maintains semantic alignment with the original BDD scenario

**PAY SPECIAL ATTENTION TO:**
- Bracket placement: All guards and conditions MUST be in `[...]`
- Gate syntax: Use `!` for system output, `?` for input
- Assignment format: Follow the pattern `var := expression`
- Line breaks: Each switch on a separate line
...
## 5. CRITICAL REMINDERS FOR RETRY MODE

**BEFORE YOU GENERATE THE TRANSLATION:**
1. ✓ Review the error analysis above carefully
2. ✓ Identify the exact location and nature of the error
3. ✓ Check the correction suggestion provided
4. ✓ Apply the fix while maintaining semantic correctness

**COMMON MISTAKES TO AVOID:**
- Missing brackets around guards or conditions
- Incorrect gate syntax (using wrong direction)
- Missing assignments after conditions
- Incorrect line breaks (all switches must be on separate lines)
- Malformed expressions or function calls

---

## 6. Final Instruction

Process the input JSON and return the same JSON with the `"ibdd_representation"` field containing your **CORRECTED** IBDD translation for each scenario.
...
## 7. RETRY MODE FINAL CHECK

Before submitting your translation, verify:
- [ ] The specific error mentioned above has been fixed
- [ ] All brackets are in place for guards and conditions
- [ ] All gates have correct syntax (`!` or `?`)
- [ ] All assignments follow the correct format
- [ ] Line breaks are correctly placed
- [ ] The translation is semantically correct with respect to the original BDD
