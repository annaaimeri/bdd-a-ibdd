...
Aunque el trabajo de Karpurapu et al. aborda la dirección inversa a la de este trabajo (historias de usuario $\rightarrow$ Gherkin, frente a Gherkin $\rightarrow$ IBDD), los patrones observados son similares: los modelos GPT mantienen consistencia sintáctica superior, Llama presenta dificultades recurrentes con formalismos sintácticos, y la provisión de ejemplos (como el caso de la impresora industrial incluido en el prompt de traducción) mejora los resultados. Esta convergencia refuerza la validez de la selección realizada.

\subsection{Modelo Seleccionado}

Sobre la base de los resultados experimentales y su consistencia con la literatura, se seleccionó la familia de modelos GPT-4o de OpenAI como motor de lenguaje para el sistema de traducción. El modelo demostró corrección sintáctica en todas las traducciones generadas, alta uniformidad de calidad entre ejecuciones y capacidad de explorar enfoques alternativos genuinos.
% TODO: revisar si queremos especificar la versión exacta de GPT-4o utilizada para los experimentos.

No obstante, una decisión de diseño fundamental del sistema es su independencia respecto del modelo y del proveedor de lenguaje utilizados. Si bien GPT-4o demostró el mejor desempeño en la evaluación comparativa, la arquitectura del sistema abstrae la comunicación con el LLM mediante una capa de abstracción que parametriza tanto el modelo como el proveedor como variables de configuración. Esta abstracción permite intercambiar el modelo subyacente, ya sea entre versiones de un mismo proveedor o entre proveedores distintos, sin modificar ningún otro componente del pipeline.

Esta decisión responde a múltiples consideraciones. En primer lugar, la \textbf{soberanía de datos}: en contextos institucionales y académicos, el envío de información a servicios externos puede estar sujeto a restricciones de privacidad, regulaciones de protección de datos o políticas internas que impidan el uso de APIs propietarias en la nube. La posibilidad de ejecutar el sistema con modelos de código abierto desplegados localmente (como Llama u otros modelos compatibles a través de Ollama) permite operar el pipeline sin que los datos abandonen la infraestructura de la institución. En segundo lugar, la \textbf{reproducibilidad y continuidad}: los modelos propietarios están sujetos a actualizaciones, discontinuaciones y cambios en las condiciones de servicio por parte de sus proveedores; un sistema acoplado a un modelo específico quedaría obsoleto ante cualquiera de estos eventos. En tercer lugar, la \textbf{evolución del estado del arte}: el campo de los LLMs avanza con rapidez, y modelos de código abierto que hoy presentan limitaciones podrían alcanzar o superar el desempeño de los modelos propietarios en el corto plazo. Un sistema agnóstico al modelo puede incorporar estos avances de forma transparente, sin requerir rediseño.

En la práctica, el sistema soporta actualmente dos proveedores: OpenAI (para modelos propietarios como GPT-4o) y Ollama (para modelos de código abierto ejecutados localmente). La selección se realiza mediante parámetros de línea de comandos, lo que facilita la evaluación comparativa entre modelos y la adaptación a diferentes contextos de despliegue. La Sección~\ref{sec:modulo-traduccion} describe la implementación de esta abstracción.
...
Se seleccionó JSON como formato de intercambio entre el sistema y el modelo de lenguaje, tanto para la representación de los escenarios BDD de entrada como para las traducciones IBDD de salida. Esta decisión se fundamenta en la relación entre los datos de entrenamiento de los modelos de lenguaje y su desempeño en tareas de generación estructurada. Los modelos de la familia GPT fueron entrenados sobre corpus masivos que incluyen una proporción significativa de datos en formato JSON, provenientes de repositorios de código fuente, documentación de APIs web, archivos de configuración y conjuntos de datos abiertos~\cite{openai2023gpt4}. Dado que estos modelos observaron millones de ejemplos de JSON bien formado durante su entrenamiento, han desarrollado una familiaridad inherente con la sintaxis de este formato que se manifiesta en dos niveles. A nivel de tokenización, el algoritmo \textit{Byte Pair Encoding} (BPE) utilizado por estos modelos ha aprendido a segmentar eficientemente las estructuras sintácticas características de JSON (llaves, corchetes, comillas, dos puntos), produciendo secuencias de tokens más compactas que las obtenidas con formatos menos representados en los datos de entrenamiento~\cite{sennrich2016bpe}. A nivel de generación, el modelo produce JSON con mayor consistencia estructural y menor tasa de errores de formato que otros formatos de serialización, precisamente porque ha internalizado los patrones sintácticos de JSON a partir de la exposición extensiva durante el entrenamiento. En consecuencia, la elección de JSON como formato de intercambio maximiza tanto la eficiencia de tokenización, al reducir el consumo de tokens por solicitud, como la confiabilidad de las respuestas generadas.
...
Sin embargo, una porción significativa de la comunidad de desarrollo de software opera en español, y las especificaciones de comportamiento en entornos hispanohablantes se redactan frecuentemente en este idioma. Un sistema de traducción BDD$\rightarrow$IBDD que solo funcione con escenarios en inglés tendría una aplicabilidad limitada en estos contextos. Por este motivo, se construyó una versión en español del dataset de escenarios BDD, traduciendo los escenarios del dataset original y verificando que las traducciones preserven la estructura y la intención del escenario sin introducir ambigüedades.

La inclusión de un dataset bilingüe permite evaluar el comportamiento del sistema ante escenarios en español, lo cual constituye una contribución relevante dado que no se han encontrado trabajos previos que aborden la traducción BDD$\rightarrow$IBDD desde escenarios en un idioma distinto del inglés. Se espera que el desempeño del sistema sea superior con escenarios en inglés, dado que los LLMs fueron entrenados sobre corpus con una proporción considerablemente mayor de texto en este idioma, y que la gramática IBDD emplea palabras clave en inglés (\texttt{GIVEN}, \texttt{WHEN}, \texttt{THEN}). No obstante, la evaluación bilingüe permite cuantificar esta diferencia y determinar si el sistema mantiene un nivel de desempeño aceptable con escenarios en español.
