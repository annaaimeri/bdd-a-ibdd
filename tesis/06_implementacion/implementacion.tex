...
El sistema implementa un pipeline de cuatro etapas orquestado por la clase
...
\begin{enumerate}
  \item \textbf{Traducción} (Sección~\ref{sec:modulo-traduccion}).
  ...
  \item \textbf{Validación sintáctica} (Sección~\ref{sec:modulo-validacion}).
  ...
  \item \textbf{Análisis de errores} (Sección~\ref{sec:modulo-correccion}).
  ...
  \item \textbf{Reintento con correcciones} (Sección~\ref{sec:modulo-correccion}).
...
El pipeline completo se implementa en Python. Los datos fluyen secuencialmente desde \texttt{Dataset.json} (o desde particiones derivadas con la misma estructura) hasta los archivos de resultados finales, y todas las etapas operan sobre archivos JSON como formato de intercambio.

\section{Definición de la Gramática IBDD}

La gramática IBDD se especifica en formato EBNF compatible con Lark y constituye el componente central del módulo de validación. Esta es la gramática operacional adoptada por el sistema: toma como base la estructura general del lenguaje IBDD presentada en el Capítulo de Marco Teórico y la adapta para manejar las variaciones de formato que genera el LLM durante la traducción.
